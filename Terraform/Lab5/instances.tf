...
locals  {
  team_folder = "${var.folder}${var.team_number}/LabAnsible"
...
resource "vsphere_virtual_machine" "camlab" {
  ...
  provisioner "file" {
    ...
    destination = "/tmp/terraform-scripts"
  }
  
  # Copy ansible rpms
  provisioner "file" {
    source      = "${path.module}/ansible-install-rpms"
    destination = "/tmp/ansible-install-rpms"
  }

  # Copy ansible playbooks
  provisioner "file" {
    source      = "${path.module}/ansible-playbooks"
    destination = "/tmp/ansible-playbooks"
  }

  provisioner "remote-exec" {
    inline = [
      "sudo chmod u+x /tmp/terraform-scripts/*.sh",
      "/tmp/terraform-scripts/add-public-ssh-key.sh \"${tls_private_key.ssh.public_key_openssh}\"",
      "/tmp/terraform-scripts/add-private-ssh-key.sh \"${tls_private_key.ssh.private_key_pem}\" \"${var.ssh_user}\"",
      "/tmp/terraform-scripts/install-ansible.sh",
      "ansible-playbook /tmp/ansible-playbooks/harden-vm.yml"
    ]
  }
}
